 ...
 *
 */
public class TuneDirTheory {
	
	
	protected static final Logger logger = LoggerFactory.getLogger(TuneDirTheory.class);
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		for(int i = 0; i<muvalues.length;i++) {
			...
			PrintWriter pt = new PrintWriter(new File("var/results/res_dir_theory_" + String.valueOf(mu) + ".res"));
			...
			while (querySource.hasNext()) {
				...
				System.out.println("Scoring with Dir Theory; mu=" + mu);
				//scoring with LM dir
				Request rq = new Request();
				rq.setOriginalQuery(query);
				rq.setIndex(index);
				rq.setQueryID(qid);
				rq = tlm.runMatching(rq, "dir_theory", "dir");
				TRECoutput.printResults(pt, rq, "dir_theory", "Q0", 1000);
			}
			pt.flush();
			pt.close();
		}
