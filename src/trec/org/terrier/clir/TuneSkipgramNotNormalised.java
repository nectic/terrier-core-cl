 ...
 *
 */
public class TuneSkipgramNotNormalised {


	protected static final Logger logger = LoggerFactory.getLogger(TuneSkipgramNotNormalised.class);
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		for(int i = 0; i<muvalues.length;i++) {
			...
			PrintWriter pt_w2v_skipgram = new PrintWriter(new File("var/results/res_dir_w2v_skipgram_notnorm_" + String.valueOf(mu) + ".res"));
			...
			while (querySource.hasNext()) {
				...
				rq_w2v = tlm_w2v_skipgram.runMatching(rq_w2v, "w2v_notnorm", "dir");
				TRECoutput_w2v_skipgram.printResults(pt_w2v_skipgram, rq_w2v, "dir_w2v_skipgram_notnorm", "Q0", 1000);
	...
	public void processQueries() throws IOException, InterruptedException {
		...
		TranslationLMManager tlm_w2v_skipgram = new TranslationLMManager(index);
		System.out.println("Initialise word2vec translation");
		tlm_w2v_skipgram.initialiseW2V_atquerytime(src_we);
		System.out.println("word2vec translation initialised");
		tlm_w2v_skipgram.setTranslation("w2v");
		tlm_w2v_skipgram.setRarethreshold(index.getCollectionStatistics().getNumberOfDocuments()/200);
		tlm_w2v_skipgram.setTopthreshold(index.getCollectionStatistics().getNumberOfDocuments()/2);
