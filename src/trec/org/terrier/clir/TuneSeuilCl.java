 ...
 *
 */
public class TuneSeuilCl {


	protected static final Logger logger = LoggerFactory.getLogger(TuneSeuilCl.class);
	...
	public void processQueries() throws IOException, InterruptedException {
		...
		for(int i = 0; i<muvalues.length;i++) {
			...
			PrintWriter pt_w2v_skipgram = new PrintWriter(new File("var/results/res_seuil_cl_mu_" + String.valueOf(mu) + ".res"));
			...
			while (querySource.hasNext()) {
				...
				rq_w2v = tlm_w2v_skipgram.runMatching(rq_w2v, "w2v_seuil_cl", "dir");
				TRECoutput_w2v_skipgram.printResults(pt_w2v_skipgram, rq_w2v, "w2v_seuil_cl", "Q0", 1000);
			}
