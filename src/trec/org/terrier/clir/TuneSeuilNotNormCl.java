 ...
 *
 */
public class TuneSeuilNotNormCl {


	protected static final Logger logger = LoggerFactory.getLogger(TuneSeuilNotNormCl.class);
	...
	public void processQueries() throws IOException, InterruptedException {
		...
		for(int i = 0; i<muvalues.length;i++) {
			...
			PrintWriter pt_w2v_skipgram = new PrintWriter(new File("var/results/res_w2v_seuil_notnorm_cl_mu_" + String.valueOf(mu) + ".res"));
			...
			while (querySource.hasNext()) {
				...
				rq_w2v = tlm_w2v_skipgram.runMatching(rq_w2v, "w2v_seuil_notnorm_cl", "dir");
				TRECoutput_w2v_skipgram.printResults(pt_w2v_skipgram, rq_w2v, "w2v_seuil_notnorm_cl", "Q0", 1000);
			}
